  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {topRecs.map((rec) => {
          let badgeColor = 'bg-brand-softGreen text-brand-fresh';
          let priorityIcon = <Compass className="w-4 h-4 text-brand-fresh" />;
          ...
          if (rec.priority === 'High Priority') {
            ...
            badgeColor = 'bg-brand-lightGold border border-brand-goldBorder/50 text-brand-gold';
            priorityIcon = <Bookmark className="w-4 h-4 text-brand-gold" />;
          }

          return (
            <div
              key={rec.serviceId}
              className="bg-white rounded-2xl border border-brand-borderLight hover:border-brand-emerald/20 shadow-subtle hover:shadow-card-hover p-6 flex flex-col justify-between space-y-6 transition-all duration-300 group"
            >
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <h4 className="font-heading font-extrabold text-base sm:text-lg text-brand-charcoal leading-snug group-hover:text-brand-emerald transition-colors">
                    {rec.serviceTitle}
                  </h4>
              ...
              <div className="pt-2">
                <NavLink
                  ...
                  className="inline-flex items-center gap-2 font-heading font-bold text-xs sm:text-sm text-brand-emerald hover:text-brand-emeraldHover group-hover:translate-x-0.5 transition-all"
                >
